...
export class Game {
  ...
  async predict() {
    ...
    if (this.isRunning) {
      ...
      // provided image data.
      const topK =
          await this.emojiScavengerMobileNet.getTopKClasses(result, 1); // nh: otherwise too many items are 'recognized'

      // Match the top 2 matches against our current active emoji.
      this.checkEmojiMatch(topK[0].label, topK[0].label); // nh: otherwise too many items are 'recognized'
